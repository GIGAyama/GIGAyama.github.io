...
| **2026-08-28** | 生成物の検査が、**前に `npm run build` を置くだけで必ず緑になっていた**（10 本） | `verify-generated.mjs` は「作業ツリーのファイルを控える → 作り直す → 比べる」順で見ていた。艦隊の 10 本が `ci = npm run build && npm run verify` と書いており、控える相手が build の出力そのものになるため、**比較が恒真式**になっていた。Shared-Folder-Sync の `vendor_icons.html` にわざと 1 行足すと `npm run ci` は exit 0、`npm run verify` だけなら exit 1。つまりこの 10 本では生成物の検査は 1 度も何も見ていなかった。さらに 6 本は、その `verify` をワークフローが一度も呼んでいなかった。 | 比べる相手を**git がコミットしている中身**に変え、前に build が何回走っても結果が変わらないようにした。**順番で結果が変わる検査は、いつか必ず順番のほうを間違える。** 検査を書いたら、その検査を呼ぶ側（ワークフロー）まで見て、わざと壊して落ちることを確かめること。 |
| **2026-08-29** | `.agents/rules/gigaschool-standards.md` を **42 本へ配りながら、一度も照合していなかった** | Typa のルールに 1 行足しても `check-drift` は「✅ 正本と一致しています」で **exit 0** を返した。原因が 3 つ重なっている: `canonicalIndex` の `DISTRIBUTED_EXT` に `.md` が無い／`findLookalikes` は `.` で始まる名前を歩かない／`unregisteredSkills` は `skills/` の下しか見ない。そして 42 本のどの `standards-map.json` にも、この行が無かった。**2026-08-28 の「`.agents/skills` が無検査だった」とまったく同じ型**で、あのときスキルは塞いだが、ルールファイル自身が取り残されていた。**エージェントの行動を決める文書が、書き替え放題で緑になっていた。** | `distribute.mjs` が `files` へ自動登録する（`check-drift` の `files` ループは拡張子を見ない）。`check-drift.test.mjs` に **CLI を実際に起動する**試験を足した（純関数の試験では、比べる処理が正しく動いているこの壊れ方は捕まらない）。**同じ型が別の場所に残っていないかを機械で探せるよう `docs/architecture/lessons.json` を作った。** |
| **2026-08-29** | **Claude Code だけが常時ルールを読んでいなかった**（40 本） | `.agents/rules/…` は 43 本にあるのに `CLAUDE.md` は 3 本しかなかった。`distribute.mjs` の step 3 は Antigravity 向けにしか配っておらず、Claude Code 向けの同等物が無い。スキルは配ってあるが、スキルは「呼ばれたときだけ」読まれるので代わりにならない。 | 正本 `standards/agents/CLAUDE.md` を新設し配布。ルール本文は持たせず `@.agents/rules/gigaschool-standards.md` の 1 行で取りこむ（同じ文を 2 か所に置かない）。独自の `CLAUDE.md` を持つ 3 本は `unmanaged` に宣言し、**取りこみ 1 行の有無を `check-distribution.mjs` が見る**（宣言しただけで無検査、にしない）。 |
| **2026-08-29** | 配布のとりのこし検査が、**マージのたびに必ず赤くなっていた**（2026-08-27 以降ほぼ全部） | `check-distribution` と `auto-distribute` がどちらも `push: branches: [main]` で起動するため、**2 つが同時に走って検査のほうが必ず先に終わる**。配る前の艦隊を見るので「配っていません」が毎回出る。#104 では 検査 04:14→04:15 で失敗、配布 04:14→04:20 で成功。配布後に同じ検査を手で走らせると `✅ 42 本・2112 ファイル一致`（exit 0）。**毎回赤い検査は、赤いことに意味が無くなる**（人を「無視してよい」に慣らす点で、誰も見ない警告より悪い）。 | `workflow_run`（`types: [completed]`）にして、**配布が終わってから**見る。失敗でも発火させる（落ちた日ほど何が配られていないかを知りたい）。⚠️ `workflow_run` は**表示名**で相手を指すので、`auto-distribute` の `name:` を変えると二度と起動しないのに誰も気づかない。名前の一致を試験で固定した。 |
| **2026-08-29** | 実ブラウザ検査の「0 件を信じない」を**要求の数**で書いたら、接続できない URL が合格になった | `--url http://127.0.0.1:9/…`（接続不能）に対して「✅ 2 件の要求を記録」で **exit 0**。開けなかった navigation の要求そのものが記録されるため。**防ごうとした当の形に、検査自身がはまっていた。** | 数えるのは**成功した応答**（`status < 400`）にする。開けなかったときは理由（`net::ERR_…`）を添えて落とす。実測して見つけた穴なので、`lessons.json` に 1 件として残した。 |
...
| **2026-08-31** | 更新ログの仕組みを一式そろえた日に数えたら、**42 本中 0 本**が書いていなかった | `giga-changelog` スキルも、集める側（`sync-updates --fetch`）も、出す先 3 か所も、書式検査も CI も揃っていた。足りなかったのは**発火**だけで、スキルは「更新ログを書いて」と言われたときにしか動かない。ルール文書には「コミット前に 1 行足すこと」と書いてあったが、書かれなかった。`guard-canonical.mjs` のときと同じ「**文書のお願いだけでは止まらない**」型。 | PreToolUse の hook（`remind-changelog.mjs`）が、使う人から見て変わる直しを更新ログ無しでコミットしようとしたとき**1 回だけ**止める。hook の無い Antigravity 向けにはルール正本の §5 が担保する。⚠️ **満たすまで止めない。** 使う人から見て何も変わらない直し（CI・検査・道具）で止めつづけると、書き手は止まらないために**嘘の更新ログを足す**。それは書かれないことより悪い。⚠️ **コミットの題からは作らない**（2026-08-24 に 3 本を数えて捨てた道）。効いたかどうかは `fleet-status --todo` の「更新ログが1行も無い」の本数で見届ける。 |
| **2026-09-02** | 2026-08-28 に直したはずの入口判定が、正本 3 本と配布先 約 30 本に残っていた | `build-sw-static.mjs` `check-secrets.mjs` `check-distribution.mjs` が `` `file://${process.argv[1]}` `` を文字列で組み立てて `import.meta.url` と比べていた。Windows だけでなく、**空白や日本語を含むパスでは Linux でも一致しない**（`a%20b` と `a b`）。一致しなければ main() は呼ばれず、**何も見ないまま exit 0**——「SW の版は合っている」「秘密の直書きは無い」の緑が嘘になる。giga-reviewer を直したとき、同じ型が別の場所に残っているかを見る手だてが無かった。 | 3 本とも `pathToFileURL(process.argv[1]).href === import.meta.url` に直して配布。**同じ型を字面で見張る検査**を足した（`standards/lib/cli-entry.test.mjs`。standards/ と tools/ の CLI ぜんぶをコメントを落として走査し、1 本でも残っていれば落ちる）。配布先固有の道具（XXX_automatic の scripts/ 10 本ほか）も同じ形に直した。 |
| **2026-09-02** | CI の Zero-CDN 検査に、2026-08-28 の 3 つの穴がそのまま残っていた | 08-28 に直したのはエージェント用の `lint-giga.mjs` だけで、**18 本の CI を守る `B_NO_CDN_CODE`（giga-v5-checks.mjs）は `https?://` の `<script>` `<link>` `<iframe>` しか見ていなかった**。スキームを省いた `//cdn…`、`<img src>`、CSS の `@import` / `url()`（書体・画像）、ESM の `import 'https://…'`、SW の `importScripts` は緑のまま通った。同じ教訓を 2 か所の検査に落とすとき、片方だけ直して終わっていた。 | `B_NO_CDN_CODE` を「外のホストから読む資産が 0 件」に広げた（HTML の読みこみ要素ぜんぶ＋srcset／JS の import・Worker・setAttribute／CSS の @import・url()／`<style>` と JS が組み立てる印刷ウィンドウの中／SW）。`allowedRemoteScripts` はそのまま効く。3 つの穴をそれぞれテストで固定した。**教訓の台帳（lessons.json）の 08-28 の行に、CI 側の検査も guardedBy として書いた。** |
| **2026-09-02** | 正本配布が「材料だけ新しく、作ったものは古いまま」の main を 13 本作っていた | `distribute.mjs` は写して `git add .` するだけだった。ところが写しの中には配布先で**別のものの材料**になっているものがある（`web/giga-app-links.js` は静的 PWA の sw.js が先読みし、GAS のアプリでは `giga_links.html` に焼きこまれる。`records-export.*` も先読みに載る）。2026-08-30 の配布で 6 本が `build-sw --check`、7 本が `build-app-links --check` で赤くなった。赤いだけならまだよい。**sw.js の版が変わらないので、子どもの端末には新しい中身が一度も届かない**（2026-08-21 と同じ型。今度は人ではなく配布の機械が上げ忘れていた）。gh.mjs のコメントは「配布は公開される HTML・CSS・JS に一切触れない」と書いていたが、事実と違っていた。 | 配ったあとに `tools/lib/distribute-regenerate.mjs` が「在る道具」から作り直すものを決め、`build-app-links.mjs` → 静的型の `build-sw.mjs` の順で走らせてから `git add .` する。作り直しに失敗したリポジトリには配らず、作業ツリーを配る前へ戻す（材料だけ新しい main を作らない）。**リポジトリ名の一覧は持たない**（持つと 13 本目で表だけ古くなる）。13 本の main は手で作り直して直した。 |
