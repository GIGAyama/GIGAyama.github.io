...
- **何の時間に使うアプリかを、題のすぐ下に出します。** 教科・分野（`data/apps.json` の
  `category`）と、つかいかた（トップページのカードの `data-use`）をチップにして、
  押すとトップの同じ分類だけが出ます。表示名の正本は `tools/lib/categories.mjs` で、
  index.html の選択肢とずれていないかは `node tools/check-cards.mjs` が見ています
